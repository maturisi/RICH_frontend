...
void rich_init_regs(MAROC_Regs *regs)
{
	...
	regs->Global0.bits.polar_discri = 1;
...
void rich_dump_scalers()
{
	...
	rich_print_scaler("Or0_1", rich_read32(&pRICH_regs->Sd.Scaler_Or1[0]), ref);
	rich_print_scaler("Or0_2", rich_read32(&pRICH_regs->Sd.Scaler_Or2[0]), ref);
	rich_print_scaler("Or0_3", rich_read32(&pRICH_regs->Sd.Scaler_Or3[0]), ref);
	rich_print_scaler("Or1_1", rich_read32(&pRICH_regs->Sd.Scaler_Or1[1]), ref);
	rich_print_scaler("Or1_2", rich_read32(&pRICH_regs->Sd.Scaler_Or2[1]), ref);
	rich_print_scaler("Or1_3", rich_read32(&pRICH_regs->Sd.Scaler_Or3[1]), ref);
	...
	for(j = 0; j < 64; j++)
	{
		...
		for(i = 0; i < 3; i++)
		{
			...
			fval = (float)val / ref;
			printf(" [%10u,%13.3fHz,%04X]", val, fval, &pRICH_regs->MAROC_Proc[i].Scalers[j]);
		}
		printf("\n");
	}
